...
provision_leg() { # name compose_path env... — progress on stderr, uuid on stdout
	...
	for kv in "$@"; do
		case "${kv%%=*}" in
		CORE_PORT | GATEWAY_PORT | CORE_BIND | GATEWAY_BIND | GATEWAY_URL | RENDERER_URL | CORE_API_URL | EMBEDDING_BACKEND | EMBEDDINGS_INSTALL_MODEL | MODEL_T1 | MODEL_T2)
...
US_UUID=$(provision_leg "rpim-us-leg" "/docker-compose.us.yml" \
	...
	"MODEL_T1=gemini:gemini-2.5-flash" \
