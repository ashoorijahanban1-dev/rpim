...
def test_export_version_is_five(client: TestClient):
    """GET /export → export_version == 5 (M23a added origin + agent_actions)."""
    ...
    body = resp.json()
    assert body.get("export_version") == 5, (
        f"export_version must be exactly 5 (int), got: {body.get('export_version')!r}"
    )
...
def test_export_e2e_roundtrip(client: TestClient):
    ...
    has campaign_code and status='sent'; export_version == 5.
    """
    ...
    # ---- export_version == 1 ----
    assert body.get("export_version") == 5, (
        f"export_version must be 5 in e2e response, got: {body.get('export_version')!r}"
    )
