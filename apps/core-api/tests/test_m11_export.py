...
def test_export_version_is_two(client: TestClient):
    """GET /export → export_version == 2 (M20 added brain meta + chunk kinds)."""
    ...
    body = resp.json()
    assert body.get("export_version") == 2, (
        f"export_version must be exactly 2 (int), got: {body.get('export_version')!r}"
    )
...
def test_export_e2e_roundtrip(client: TestClient):
    ...
    has campaign_code and status='sent'; export_version == 2.
    """
    ...
    # ---- export_version == 1 ----
    assert body.get("export_version") == 2, (
        f"export_version must be 2 in e2e response, got: {body.get('export_version')!r}"
    )
